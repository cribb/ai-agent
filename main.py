...
def main():
    ...
    i,N = 0,MAX_AI_ITERATIONS
    while True:
        i += 1
        if i > N:
            print(f"Iteration limit ({N} reached. Exiting now...)")
            sys.exit(1)
        
        try:
            final_course = gemini_meat(client, messages, verbose)
            if final_course:
                print(f"Gemini's last words (from {i} iterations):")
                print(final_course)
                break
        except Exception as e:
            print(f"Error from content generated by Gemini:\n {e}.")

    # gemini_meat(client, messages, verbose)



def gemini_meat(client, messages, verbose):
    ...
    for part in func_calls:
        ...
        func_responses.append(result.parts[0])
